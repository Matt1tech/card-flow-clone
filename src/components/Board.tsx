...
export default function Board() {
  ...
  const handleDragOver = (e: DragEvent<HTMLDivElement>) => {
    ...
    const targetCardElement = findClosestCardElement(
      e.target as HTMLElement,
      targetListId
    );
    if (targetCardElement) {
      targetCardElement.classList.add("drag-over");
      dragOverCardIndex.current = parseInt(
        targetCardElement.getAttribute("data-card-index") || "-1"
      );
    } else {
  ...
  const findClosestCardElement = (
    ...
    while (element) {
      if (
        ...
        element.getAttribute("data-list-id") === listId
      ) {
        return element;
      }
  ...
  return (
    <div className="flex-1 overflow-x-auto p-6 bg-background dark:bg-background board-container">
      <div className="flex items-start h-full min-h-[80vh] w-max">
        <ResizablePanelGroup
          direction="horizontal"
          className="flex space-x-4 items-start h-full"
        >
          {sortedLists.map((list, index) => (
            <React.Fragment key={list.id}>
              <ResizablePanel
                ...
                onDragOver={(e) =>
                  handleListDragOver(e as unknown as DragEvent<HTMLDivElement>)
                }
                onDrop={(e) =>
                  handleListDrop(
                    e as unknown as DragEvent<HTMLDivElement>,
                ...
                onDragEnd={(e) =>
                  handleListDragEnd(e as unknown as DragEvent<HTMLDivElement>)
                }
              >
                <List
                  list={list}
              ...
              {index < sortedLists.length - 1 && <ResizableHandle withHandle />}
            </React.Fragment>
          ))}
          <div className="ml-4 flex-shrink-0 min-w-[270px] max-w-[350px]">
            <AddList />
          </div>
        </ResizablePanelGroup>
      </div>
